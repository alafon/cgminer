...
libhex8_get_target (double diff)
{
  ...
  //applog(LOG_ERR, "diff = %12.10g = 0x%08x", diff, nBits);
  return nBits;
}
                           ...
                           uint32_t nonce, uint8_t work_id, bool diff1)
{
  ...
  diff = work_sub->work_difficulty;
  if (test_nonce (work_sub, nonce))
    {
      submit_tested_work_no_clone (thr, work_sub, diff1);
      if (diff1)
        return 1;
      return (int) diff;
    }
