...
libhexb_usb_dead (struct cgpu_info *hexminerb)
{
  ...
  //if(ret)
  //applog(LOG_ERR,"info->shut_read  %i info->shut_write %i info->shut_reset %i",info->shut_read , info->shut_write , info->shut_reset);  
...
static void
libhexb_set_word (struct cgpu_info *hexminerb, uint16_t address,
                  uint16_t word)
{
  ...
  libhexb_csum (status, status + 7, status + 7);
  libhexb_sendHashData (hexminerb, status, 8);
}
